...
export default function DataDisplay({
      ...
      {filteredData.length > 0 && (
        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-0 md:justify-between px-4 py-3 text-sm border-b border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/30">
          <div className="flex items-center gap-2">
            <span className="text-gray-700 dark:text-gray-200">{t.rowsPerPage}</span>
            <select
              value={pageSize}
              onChange={(e) => {
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{t.export}:</span>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                title={t.exportCurrentPageAsCsv}
              >
              ...
              <button
                ...
                title={t.exportCurrentPageAsPdf}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 relative z-0">
      {/* Simplified Header */}
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">
          {selectedDevice 
            ? `${t.dataFor} ${deviceList.find(d => d.id === selectedDevice)?.name || selectedDevice}`
            : `${t.allDevicesData} (${new Date(selectedDate).toLocaleDateString('pt-PT')})`}
        </h2>
        ...
        {isHistoricalView && (
          <div className="text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-md border border-blue-200 dark:border-blue-700">
            {t.viewingHistoricalData} ({filteredData.length} {t.records})
          </div>
        )}
      </div>
        ...
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">{t.totalRecords}</p>
              <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{filteredData.length}</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">{t.totalPower}</p>
              <p className="text-xl font-bold text-blue-600 dark:text-blue-400">{Math.round(totalPower)} W</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">{t.averagePowerLabel}</p>
              <p className="text-xl font-bold text-green-600 dark:text-green-400">{Math.round(avgPower)} W</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">{t.devices}</p>
      ...
      {isLoading ? (
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">{t.loadingConverterData}</p>
        </div>
      ) : filteredData.length === 0 ? (
        <div className="text-center py-8">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{t.noDataAvailable}</h3>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {t.noDataAvailableForDevice}
          </p>
        </div>
      ) : (
        <div>
